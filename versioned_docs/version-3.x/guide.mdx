...
:::tip 提示
这篇教程适合对喜欢边学边做或小程序开发完全没有了解的开发者，如果你倾向于按部就班地学习，请把 [文档](./README.md) 的内容全部过一遍。当你看完 [文档](./README.md) 就可以发现：本篇教程就是文档的渐近式索引。
:::
...
你可能会注意到，不管是 <ReactIcon /> 还是 <VueIcon />，两者的全局配置是一样的。这是在配置文件中，Taro 并不关心框架的区别，Taro CLI 会直接在编译时在 Node.js 环境直接执行全局配置的代码，并把 `export default` 导出的对象序列化为一个 JSON 文件。接下来我们要讲到 [页面配置](#页面组件) 也是同样的执行逻辑。

因此，我们必须保证配置文件是在 Node.js 环境中是可以执行的，不能使用一些在 H5 环境或小程序环境才能运行的包或者代码，否则编译将会失败。

:::info 了解更多
Taro 的入口组件和全局配置规范是基于微信小程序而制定的，并对全平台进行统一。
你可以通过访问 [React 入口组件](./react.md#入口组件) 和 [Vue 入口组件](./vue.md#入口组件)，以及 [全局配置](./tutorial.md#全局配置) 了解入口组件和全局配置的详情。
:::

### 页面组件

页面组件是每一项路由将会渲染的页面，Taro 的页面默认放在 `src/pages` 中，每一个 Taro 项目至少有一个页面组件。在我们生成的项目中有一个页面组件：`src/pages/index/index`，细心的朋友可以发现，这个路径恰巧对应的就是我们[全局配置](./tutorial.md#全局配置)的 `pages` 字段当中的值。一个简单的页面组件如下：
...
你可以通过访问 [React 页面组件](./react.md#页面组件) 和 [Vue 页面组件](./vue.md#页面组件) 了解全部页面钩子函数和页面配置规范。
:::
...
:::info 了解更多
Taro 文档的[跨平台组件库](./components-desc.md) 包含了所有组件参数和用法。但目前组件库文档中的参数和组件名都是针对 React 的（除了 React 的点击事件是 `onClick` 之外）。
...
:::info 了解更多
除了 CSS 预处理器之外，Taro 还支持 [CSS Modules](./css-modules.md) 和 [CSS-in-JS](./css-in-js.md)。
...
在[打包体积分析](#打包体积分析) 中我们在 [mini.webpackChain](./config-detail.md/#miniwebpackchain-1) 添加了一个 Webpack 插件，达到了打包体积/依赖分析的效果。
...
在 [CSS 工具](#CSS 工具) 我们已经使用了名为 `@tarojs/plugin-sass` 的插件来实现对 `Sass` 的支持。比起使用 Webpack 拓展编译，Taro 的插件功能不用在每个端都对 Webpack 进行配置，只要使用插件即可。
...
:::info 了解更多
除了「内置环境变量」和「统一接口的多端文件」之外，Taro 还提供了别的跨平台开发解决方案，你可以访问文档 [跨平台开发](./envs) 了解更多。
:::
...
:::info 了解更多
Taro 还支持使用小程序插件，详情可以访问文档[使用小程序原生第三方组件和插件](./taro-in-miniapp.md)。
:::
